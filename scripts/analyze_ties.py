...
def analyze_ties():
    ...
    matcher = AggregatedLicenseMatcher(db_path=db_path, enable_java=False, enable_popularity=False)
    ...
    for data in fixtures:
        ...
        if len(results) >= 2:
            ...
            if top1["license_id"] != true_id:
                ...
                true_res = next((r for r in results if r["license_id"] == true_id), None)
                if true_res:
    ...
    for item in sorted(wrong_top1_diffs, key=lambda x: x["diff"]):
        print(f"  Fixture: {item['fixture']:<20} Top1: {item['top1']:<20} Diff: {item['diff']:.4f} (True: {item['true_score']:.4f}, Top1: {item['top1_score']:.4f})")
    ...
    for item in sorted(close_diffs, key=lambda x: x["diff"])[:10]:
         print(f"  Fixture: {item['fixture']:<20} Top2: {item['top2']:<20} Diff: {item['diff']:.4f}")
